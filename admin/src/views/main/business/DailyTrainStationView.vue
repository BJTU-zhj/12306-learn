<template>
  <p>
    <a-space>
      <train-select-view v-model:value="params.trainCode" placeholder="请选择车次"/>
  ...
  <a-modal v-model:visible="visible" title="每日车站" @ok="handleOk"
    ...
    <a-form :model="dailyTrainStation" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="车次编号">
        <train-select-view v-model:value="dailyTrainStation.trainCode" placeholder="请选择车次"/>
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    //监控入站时间和出站时间自动计算停战时间
    watch(
        () => [dailyTrainStation.value.inTime, dailyTrainStation.value.outTime],
        ([newIn, newOut]) => {
          if (!newIn || !newOut) {
            dailyTrainStation.value.stopTime = "00:00:00";
            return;
          }
          ...
          dailyTrainStation.value.stopTime = `${hours}:${minutes}:${seconds}`;
        },
        { immediate: true }
    );
