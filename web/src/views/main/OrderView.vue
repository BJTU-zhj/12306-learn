...
export default defineComponent({
  ...
  setup() {
    ...
    //查询当前用户所有乘车人
    const  queryPassengerInfos=()=>{
      axios.get("/member/passenger/query-mine").then(response=>{
        let data = response.data;
        if (data.success) {
          passengerInfos.value = data.content;
          //为passengerOptions赋值
          passengerInfos.value.forEach((item)=>passengerOptions.value.push({
            label: item.name,
            value: item
          }))
        } else {
    ...
    const finishCheckPassenger=()=> {
      if (tickets.value.length > 5) {
        notification.error({description: "最多只能选择5个乘客"});
        return;
      }
      ...
      for (let i = 0; i < tickets.value.length; i++) {
        for (let j = 0; j < seatTypesTemp.length; j++) {
          if (seatTypesTemp[j].code === tickets.value[i].seatTypeCode) {
            if (seatTypesTemp[j].count < 1) {
              notification.error({description: "余票不足"});
              return;
            }
            seatTypesTemp[j].count--;
            break;
          }
        }
      }
    ...
    // 选择的座位,根据SEAT_COL_ARRAY生成对应的双排座位数据
    // {
